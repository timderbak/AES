...
namespace Aesx
{
	public class MyAES
	{
		...
		public static byte[] encrypt(byte[] inp, byte[] key)
		{
			...
			if(key.Length<32)
			{
				Console.WriteLine("Ключ недостаточной длины.Ключ будет автоматически дополнен символами с начала строки!");
				    	
			}
			if(key.Length>32)
			{
				Console.WriteLine("Размер ключа превышен!Ключ будет автоматически обрезан с конца!");
			}
			int lenght = 0;
